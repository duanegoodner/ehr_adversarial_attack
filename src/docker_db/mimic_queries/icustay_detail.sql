...
,
  adm.admittime,
  adm.dischtime,
  date_part('day', adm.dischtime - adm.admittime) as los_hospital --, DATETIME_DIFF(adm.dischtime, adm.admittime, DAY) as los_hospital
,
  date_part('year', ie.intime - pat.dob) as admission_age --, DATETIME_DIFF(ie.intime, pat.dob, YEAR) as admission_age
,
  adm.ethnicity,
  case
    when ethnicity in (
...
,
  ...
  end as ethnicity_grouped -- , 'OTHER' --   1512
...
,
  ie.intime,
  ie.outtime,
  date_part('day', ie.outtime - ie.intime) as los_icu --, DATETIME_DIFF(ie.outtime, ie.intime, DAY) as los_icu
,
  DENSE_RANK() OVER (
...
ORDER BY ie.subject_id,
  adm.admittime,
  ie.intime
) to '/mimic_query_results/icustay_detail.csv' with delimiter ',' csv header;;
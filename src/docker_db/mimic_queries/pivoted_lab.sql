...
select
	...
	outtime
    ,
	lag (outtime) over (partition by subject_id
order by
	intime) as outtime_lag
    ,
	lead (intime) over (partition by subject_id
order by
	intime) as intime_lead
from
	mimiciii.icustays
) ,
i_vsep as
...
select
	...
	case
		...
		and i_vsep.time_since_last_discharge < (interval '24 hours')
			then i_vsep.intime - (i_vsep.time_since_last_discharge) / 2
		else (i_vsep.intime - interval '12 hours')
	end as data_start
	,
	case
		when i_vsep.time_until_next_admission is not null
		and i_vsep.time_until_next_admission < (interval '24 hours')
			then i_vsep.outtime + (i_vsep.time_until_next_admission) / 2
...
select
	...
	dischtime
    ,
	lag (dischtime) over (partition by subject_id
order by
	admittime) as dischtime_lag
    ,
	lead (admittime) over (partition by subject_id
order by
	admittime) as admittime_lead
from
	mimiciii.admissions
),
h_vsep as
...
select
	...
	case
		...
		and h_vsep.time_since_last_discharge < (interval '24 hours')
			then h_vsep.admittime - (h_vsep.time_since_last_discharge) / 2
		else (h_vsep.admittime - interval '12 hours')
	end as data_start,
	case
		when h_vsep.time_until_next_admission is not null
		and h_vsep.time_until_next_admission < (interval '24 hours')
			then h_vsep.dischtime + (h_vsep.time_until_next_admission) / 2
...
from
  ...
  (
    ...
    51300
		-- WBC COUNT | HEMATOLOGY | BLOOD | 2371
  )
